...
import type { Flashcard, PolishStore, View, Direction, Quality } from "./types";
import {
  createFlashcard,
  calculateSM2,
...
export const useStore = create(
  persist(
    immer<PolishStore>((set, get) => ({
      ...
      addFlashcard: (polish: string, russian: string, context?: string) => {
        set(state => {
        ...
        set(state => {
          ...
          if (index !== -1) {
            ...
            if (updates.russian !== undefined) {
              state.flashcards[index].russian = updates.russian.trim();
            }
            if (updates.context !== undefined) {
              state.flashcards[index].context = updates.context.trim() || undefined;
            }
          }
        });
      },
